...
> 기획서: [PLAN.md](PLAN.md) · 최종 수정 2026-08-07
...
- [x] 재사용할 모듈 결정 — `config.py` · `llm.py` 는 가져와 고쳤고 나머지는 새로 썼다 ([PLAN §7](PLAN.md))
...
[PLAN §3](PLAN.md)의 완료 기준 7개와 대응한다. 결과는 [PLAN §7.5](PLAN.md).

### 5.1. 단일 실행 확인 ✅

- [x] 질문 하나로 V1·V2 SQL 과 두 결과
- [x] V1 결과가 음수인가 — **blue / −190,571**, 랩 문서의 값과 일치
- [x] 검토 + V2 SQL 이 한 JSON 에서 파싱되는지
- [x] 중간 산출물 전부 표시·저장
- [x] 문법 오류 SQL 이 조용히 넘어가지 않는지

### 5.2. 조건별 성공률 — 완료 기준 4 ✅

- [x] `--all-conditions` · **조건별로 반복 횟수를 다르게** 준다
      (temp 0 은 5회, temp 1.0 은 10회 — 결정적인 쪽에 10회를 쓰면 같은 답을 열 번 받는다)
- [x] 산출물 `runs/{시각}_{라벨}/{조건}/{회차}/` + `summary.json`
...
- [x] **`z > y` 방향성** — 0/5 대 10/10
- [x] **`w` vs `y` 가 핵심 비교** — `feedback-t0` 5/5.
      온도를 맞추고도 통과하므로 **차이는 온도가 아니라 실행 결과**
- [x] temp 0 인 세 조건 모두 만장일치 — 예상대로
...
- [x] `--compare-models` · **V1 을 고정**해 네 모델에 같은 것을 검토시킨다.
      생성 모델까지 바꾸면 "고칠 것" 자체가 달라져 교란이 된다.
      랩의 주장도 생성이 아니라 **검토**에 대한 것이다
- [x] **육안이 아니라 성공률로** 비교 — 판정기가 있으므로 가능하다
...
- [x] **성적표에 파싱 실패를 표시**한다 — `0/3` 만 보면 반성을 못 한 것으로 읽힌다
- [x] `rescore_without_fences()` — 저장된 원문으로 재채점 (새 호출 없음)
- [x] 랩의 *"gpt-4.1 이 self-reflection 에 가장 좋다"* 확인 —
      **지지되지만 측정된 것의 상당 부분이 JSON 형식 준수다**
- [x] `README.md` 에 실제 숫자 채우기

> ⚠ **랩은 SQL 의 펜스는 벗기고 검토 응답의 펜스는 벗기지 않는다.** 한쪽만 처리한다.
> `_parse_review` 는 고치지 않았다 (규칙 1) — 펜스 제거는 B 후보.
...
[`notes/retrospectives/sql-agent.md`](../../notes/retrospectives/sql-agent.md)

- [x] 조건 4개의 실제 성공률 — 특히 `feedback-t0` vs `text`
- [x] 모델별 정확도 — **펜스 때문에 0점이 된 것을 구분해서** 기록
- [x] **공통 모듈 추출 판단** — 실제로 재봤다

| 모듈 | 공통 | 판단 |
|---|---:|---|
| `trace.py` | 72% | 추출 가치 있음 |
| `config.py` | 61% | 추출 가치 있음 |
| `llm.py` | 41% | 일부만 (chart 쪽 이미지 경로) |
| `report.py` | 20% | 사실상 다름 (보여줄 것이 다르다) |

**지금은 추출하지 않는다** — 진짜 공통은 100줄 남짓인데 이 워크스페이스는 패키징이
없어 경로 조작이 붙는다. **세 번째 사례가 나올 때가 적기다.** 무엇이 공통인지는
회고에 적어뒀으니 그때 다시 재지 않아도 된다.
