...
각 조건을 여러 번 돌려 성공률을 낸다. **검증하는 것은 조건 간 방향성이지
특정 실행의 성패가 아니다.**

**반복 횟수는 조건마다 다르게 준다.** 넷 중 셋이 `temperature=0` 이라 거의
결정적이므로, 거기에 10회를 쓰면 같은 답을 열 번 받는다. 호출은 실제로 흔들리는
쪽에 쓴다.

| 조건 | temp | 반복 |
|---|---|---:|
| `none` · `text` · `feedback-t0` | 0 | 5 |
| `feedback` | 1.0 | 10 |

temperature 0 인 조건이 5회 안에 갈리면 그것 자체가 기록할 발견이다.
...
1. **앞 개행과 뒤 들여쓰기도 프롬프트의 일부다.** f-string 몸통이 그대로 가므로
   `generate_sql` 은 `'\n    You are…'` 로 시작해 `'…SQL only.\n    '` 로 끝난다.
   아래 블록은 읽기 좋으라고 다듬은 것이니 **바이트 기준은 코드**다.
2. `refine_sql` 소스의 `{{` / `}}` 는 f-string 이스케이프라 모델에게는 `{` / `}` 로
   간다. **비교는 렌더한 뒤에 해야 한다** — 소스끼리 비교하면 그 두 줄에서 헛되이 실패한다.

구현은 [`sqlgen.py`](sql_agent/sqlgen.py) 의 템플릿 상수이고,
`check_prompts()` 가 노트북 원문을 렌더해 대조한다. 세 개 모두 일치를 확인했다.
...
| 추가 | 이유 |
|---|---|
| `has_error` 플래그 | 없으면 `error` 한 줄이 "최종 답" 으로 표시되고 판정기가 성공으로 센다 (§6.2 규칙 1) |
| 읽기 전용 연결 | 측정 기준 DB 를 지킨다. ⚠ 랩은 쓰기 연결이라 `DROP` 이 실제로 실행된다 |
| 쿼리 타임아웃 | 배치 실행에서 카티션 조인 하나가 수백 개 쿼리를 멈춰세우지 않도록 |

뒤 둘은 실패해도 **오류 프레임으로 나가므로** 모델에게는 다른 실패와 똑같이 피드백된다.
랩 `execute_sql` 과 출력을 대조해 7개 케이스에서 프레임·마크다운이 동일함을 확인했다.
...
| 반복 실행 | **조건별로 다르게** (temp 0 은 5, temp 1.0 은 10) | §3 |
...
기존 프로젝트에 같은 역할의 모듈이 있어 **가져다 고쳐 쓴다.** 지금 공통 모듈로 뽑으면
추측이 되므로, 동작하는 것을 만든 뒤 실제로 무엇이 같았는지 보고 결정한다.

| 모듈 | 상태 |
|---|---|
| `config.py` · `llm.py` | 가져와 고쳤다 — 온도 상수를 더하고 이미지 경로를 뺐다 |
| `trace.py` · `report.py` · `runs/` 규약 | 4단계 |
| `dataset.py` · `executor.py` · `sqlgen.py` | 새로 썼다 — SQL 이라 성격이 다르다 |

**따라서 "복사" 라기보다 골격만 같다.** 4단계가 끝나면 실제로 무엇이 같았는지 세어본다.

## 7.5. 측정 결과 (2026-08-06)

질문 0 *"Which color of product has the highest total sales?"*
...
**`feedback-t0` 가 이 표의 결론이다.** 온도를 `text` 와 같게 맞추고도 5/5 이므로,
차이는 온도가 아니라 **실행 결과**에서 온다. 랩의 구성만으로는 할 수 없던 말이다.

분산은 예상대로였다 — temp 0 인 세 조건 모두 만장일치.

### 모델별 — 완료 기준 7

같은 V1 과 같은 실행 결과를 네 모델에 줬다.
...
⚠ **왼쪽 열은 반성 능력이 아니다.** `gpt-4o` 는 부호 문제를 정확히 짚고
`SUM(ABS(qty_delta) * unit_price)` 를 냈으나, 응답을 ` ```json ` 으로 감싸
파싱이 실패했고 랩의 폴백이 원본 SQL 을 유지했다.

**랩은 SQL 의 펜스는 `execute_sql` 에서 벗기지만 검토 응답의 펜스는 벗기지 않는다.**
한쪽만 처리한다. 그래서 *"gpt-4.1 이 self-reflection 에 가장 좋다"* 는 랩의 주장은
우리 데이터로 지지되지만, **측정된 것의 상당 부분이 JSON 형식 준수**다.

`_parse_review` 는 고치지 않았다 (규칙 1). 대신 `rescore_without_fences()` 가
저장된 원문으로 재채점해 두 숫자를 함께 낸다. 펜스 제거는 B 후보다.

실제로 못 고친 것은 `gpt-3.5-turbo` 하나인데, 이것도 *"−190571 for blue seems
incorrect"* 라고 **지적은 하고 SQL 은 그대로 뒀다** — 진단은 했으나 처방을 못 한 경우다.
