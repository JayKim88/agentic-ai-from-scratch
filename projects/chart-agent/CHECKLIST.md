...
- [x] **`utils.py` 대조** — [회고 §8](../../notes/retrospectives/chart-agent-lab-findings.md)
- [x] 대조에서 나온 결함 수정 — Anthropic 텍스트 블록 유실
- [ ] B1이 값을 했는지 판정 ← 6단계 후
- [ ] 모듈 1과의 코드 중복 검토
...
| 항목 | 우리 | 랩 | 판단 |
|---|---|---|---|
| 추상화 | 텍스트만 aisuite | **provider SDK 직접** | 랩이 단순 |
| OpenAI 이미지 | Chat Completions | **Responses API** | 둘 다 동작 |
| 블록 순서 | 텍스트 → 이미지 | 동일 | 일치 |
| **여러 텍스트 블록** | ❌ 첫 것만 | ✅ 전부 | **랩이 옳다 → 수정함** |
...
**가장 크게 빗나간 추론:** 랩은 aisuite 를 아예 쓰지 않는다. 우리가 *"랩도 같은 결론에
도달했다"* 고 적었던 서술은 틀렸다 — 랩은 그 추상화를 시도조차 하지 않았다.

**고친 것:** aisuite 는 Anthropic 응답에서 `content[0].text` 만 읽는다. 여러 블록으로
나뉘어 오면 뒤를 잃고, 첫 블록이 텍스트가 아니면 깨진다. Anthropic 이미지 호출만
SDK 직접 호출로 바꿔 전부 이어붙인다.
