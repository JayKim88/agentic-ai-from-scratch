...
모듈 1과 달리 **랩 노트북의 HTML 내보내기(`M2_UGL_1.html`, 340KB)를 확보했다.**
...
랩 3.1의 **산문은 스키마를 틀리게 적어놨다.**

> *"The dataset includes fields such as `date`, `coffee_type`, `quantity`, and `revenue`"*

프롬프트에 실제로 들어가는 9컬럼에는 `coffee_type`도 `quantity`도 `revenue`도 **없다.**
`coffee_name`과 `price`가 있을 뿐이다. **프롬프트 쪽이 진실**이고 산문이 낡았다.

따라서 데이터 생성기의 제약이 확정된다.

| | |
|---|---|
| 한 행의 의미 | **거래 1건** (수량 컬럼이 없으므로 1행 = 1잔) |
| "sales" 계산 | `price` **합계** 또는 행 수(count) |
| 근거 | 랩 V1 코드가 `groupby('coffee_name')['price'].sum()` 을 씀 |

즉 판매량을 늘리려면 **행을 늘려야 한다.** `quantity` 컬럼을 만들어 넣으면
랩 프롬프트의 스키마와 어긋나고 생성 코드가 없는 컬럼을 참조하게 된다.
