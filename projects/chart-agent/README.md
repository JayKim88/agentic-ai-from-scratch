...
> 랩 산문은 *"fields such as `date`, `coffee_type`, `quantity`, `revenue`"* 라고 적었지만
> 프롬프트에 들어가는 실제 9컬럼에는 셋 다 **없습니다.** 프롬프트 쪽이 진실입니다.
> 판매량은 `price` 합계나 행 수로 셉니다 — 랩 V1 코드도 `groupby(...)['price'].sum()` 입니다.
