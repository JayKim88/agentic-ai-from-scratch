...
가장 뜻밖의 발견은 공식 구현이 **강의가 가르친 리플렉션 분해를 오히려 합쳐놓았다**는 점이다.
...
| 검토 | editor가 비평+수정을 **한 번에** | 비평/수정 **분리** (2단계) |
...
|---|---|---|
| 리플렉션 = 초안/파악/수정 3단계 분해 | 합쳐놓음 (1단계) | 분리 ✅ |
...
| 우선순위 | 항목 | 근거 |
|---|---|---|
| ~~**높음**~~ **완료** | arXiv PDF 본문 추출 | 학술 주제에서 근거 깊이가 명확히 부족 |
...
### 후속 — arXiv PDF 본문 추출 반영 결과 (2026-08-03)

구현 후 학술 주제로 실행하니 **예상 못한 것 세 가지**가 드러났다.

| 관찰 | 판정 |
|---|---|
| 초록 1,323자 → 본문 5,000자 (**3.8배**), 상위 3건만 추출해 5.1초 | 의도대로 동작 |
| "소스 다양성" eval이 실패 — 18개 논문이 전부 `arxiv.org` | **평가의 결함**. 집합체(arXiv·PubMed)에서 도메인 수는 출처 독립성의 대리 지표로 깨진다. 인용된 **출처 수**로 기준을 바꿨다 |
| arXiv 검색이 `reflection ... error correction` 질의에 **양자 오류정정 논문**을 반환 | **미해결**. arXiv API의 `all:` 검색은 단순 키워드 매칭이라 관련성이 낮다. 수집 18건 중 4건만 인용된 원인으로 보인다 |

또 하나: 학술 주제에서 모델이 **6회 전부 `search_arxiv`만** 호출했다. 웹·위키를 쓰지 않았다.
주제 성격상 합리적이지만, 도구 선택이 한쪽으로 쏠릴 수 있다는 것은 기록해둘 만하다.

### 후속 2 — arXiv 검색 관련성 개선 (2026-08-03)

`all:` 대신 `abs:`를 쓰거나 `sortBy=relevance`를 주는 것은 **아무 효과가 없었다**
(세 전략의 상위 5건이 완전히 동일). 실질적 지렛대는 **카테고리 필터** 하나뿐이었다.

`search_arxiv`에 `category` 파라미터를 열어 모델이 정하게 했다. 결과:

| | 이전 | 이후 |
|---|---|---|
| 도구 사용 | `search_arxiv` 6회 (편식) | arXiv + web + wikipedia **3종 모두** |
| 수집 소스 | 18건 중 4건만 인용 | 10건 중 10건 인용 |
| 소스 다양성 | 1개 도메인 | 5개 도메인 |
| 상위 결과 | 양자 오류정정 논문 | LLM 에이전트 논문 |

**도구 편식(관찰 ③)이 함께 해소됐다.** arXiv 결과 품질이 올라가자 모델이 다른 도구도 쓰기
시작했다 — 앞서 [ESTIMATE]로 적었던 예상이 맞았다.

한계도 확인했다: arXiv는 `all:x AND cat:bogus`에도 76,011건을 반환한다. **AND가 엄격하지 않아
`cat:`은 하드 필터가 아니라 순위 편향이다.** 질의를 인용부호로 묶으면 엄격해지지만 정확한
구절 일치를 요구해 대부분 0건이 된다.

### 후속 3 — 인용 정합성 오탐 (2026-08-03)

개선 후 실행에서 **처음으로 인용 정합성이 실패**했다(9/10). 조사해보니 평가의 오탐이었다.

- 수집된 것: `https://arxiv.org/pdf/2405.06682` (Tavily가 준 PDF 링크)
- 인용된 것: `http://arxiv.org/abs/2405.06682` (초록 페이지)

**같은 논문의 다른 경로 형태.** 버전 접미사(`v1`, `v3`)도 같은 문제를 일으킨다.
arXiv URL을 논문 ID로 정규화하도록 고쳤다. 지어낸 ID는 여전히 매칭되지 않으므로
지표의 목적은 훼손되지 않는다. → 10/10 회복.

앞서 http/https 정규화에서 겪은 것과 **같은 부류의 결함**이다.
URL 동일성 판정은 생각보다 자주 틀린다.

### 후속 4 — 가독성 해결 (2026-08-03)

Flesch 점수의 입력은 둘이다: **문장 길이**와 **단어당 음절 수**.
1차 `PROSE_RULES`는 문장 길이만 제약했고, 그것으로 비즈니스 주제는 20.4까지 올랐지만
학술 주제는 9.2에 머물렀다. 논문 본문을 넣으면 어휘가 라틴계 다음절로 끌려간다.

두 번째 입력을 겨냥해 세 줄을 추가했다.

- 독자 명시: *"Write for a working engineer who is not a specialist in this field."*
- 어휘 교체: *"use" not "utilise", "show" not "demonstrate"*
- *"Do not copy the phrasing of the sources. Say it in your own plainer words."*

비평 단계 체크리스트에도 *"짧은 일상어로 될 자리에 쓰인 라틴계 장문어"* 를 넣었다.

| 주제 | 이전 | 이후 |
|---|---|---|
| 비즈니스 (로켓 회사) | 20.4 | **30.0** |
| 학술 (reflection pattern) | 9.2 | **40.0** |

두 주제 모두 **8/8 통과**. 학술 주제의 상승폭(+30.8)이 더 컸다 — 원래 어휘 문제가 심했다는
뜻이다. 부수 효과로 검토 단계의 변경률이 11.5% → 55%로 뛰었다. 비평이 지적할 거리가
늘어나자 수정 단계가 실제로 더 많은 일을 했다.

**교훈:** 지표가 무엇으로 계산되는지 알고 나서야 고칠 수 있었다. "가독성이 낮다"를
"문장이 길다"로만 읽었을 때는 절반만 고쳐졌다.

**미해결 관찰:** 학술 주제의 인용 소스가 10건 → 5건으로 줄었다. 같은 실행에서 비즈니스
주제는 21건을 유지했으므로 실행 편차일 가능성이 높지만, 확인하지 않았다.
